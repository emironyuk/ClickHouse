...
## Синтаксис присваивания привилегий {#grant-privigele-syntax}
...
`WITH GRANT OPTION` разрешает пользователю или роли выполнять запрос `GRANT`. Пользователи могут выдавать привилегии из числа тех, что им присвоены.


## Синтаксис назначения роли {#assign-role-syntax}
...
Для использования `GRANT` пользователь должен иметь привилегию `GRANT OPTION`. Пользователь может выдать привилегию только из присвоенных его аккаунту.
...
Также у `john` есть привилегия `GRANT OPTION`. `john` может выдать другим пользователям все присвоенные ему привилегии или их часть.
...
Вы можете выдать несколько привилегий нескольким пользователям в одном запросе. Запрос `GRANT SELECT, INSERT ON *.* TO john, robin` позволит пользователям `john` и `robin` выполнять `INSERT` и `SELECT` над всеми таблицами всех баз данных на сервере.


## Привилегии {#grant-privileges}

Привилегия — это разрешение на выполнение определенных типов запросов.

Привилегии имеют иерархическую структуру. Набор разрешенных запросов зависит от уровня привилегии.
...
- `VIEW` — Привилегия присваивается для view, базы данных или глобально.
- `DICTIONARY` — Привилегия присваивается для словаря, базы данных или глобально.
- `DATABASE` — Привилегия присваивается для столбца или глобально.
...
Присваивает пользователю или роли все привилегии на регулируемый объект доступа.
...
[Original article](https://clickhouse.tech/docs/ru/query_language/grant/) <!--hide-->
