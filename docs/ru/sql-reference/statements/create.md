...
Пользовательский хост — это хост, с которого можно установить соединение с сервером ClickHouse. Хост можно задать в разделе `HOST` следующими способами:

- `HOST IP 'ip_address_or_subnetwork'` — Пользователь может подключиться к серверу ClickHouse только с указанного IP-адреса или [подсети](https://ru.wikipedia.org/wiki/Подсеть). Примеры: `HOST IP '192.168.0.0/16'`, `HOST IP '2001:DB8::/32'`. При использовании в эксплуатации указывайте только элементы `HOST IP` (IP-адреса и маски), так как использование `host` и `host_regexp` может привести к дополнительной задержке.
...
Создать аккаунт `john` и сделать все его будущие роли ролями по умолчанию:
...
Создать аккаунт `john` и сделать все его будущие роли, кроме `role1` и `role2`, ролями по умолчанию:
...
Пользователь может иметь роли по умолчанию, которые применяются при входе пользователя в систему. Чтобы задать роли по умолчанию, воспользуйтесь инструкциями [SET DEFAULT ROLE](misc.md#set-default-role-statement) или [ALTER USER](alter.md#alter-user-statement).

Отозвать роль можно с помощью инструкции [REVOKE](revoke.md).

Для удаления роли используйте инструкцию [DROP ROLE](misc.md#drop-role-statement). Удаленная роль автоматически отзывается у всех пользователей, которым была назначена.
...
Назначение аккаунту `mira` роли `accountant`:

```sql
GRANT accountant TO mira;
```

После назначения роли пользователь может выполнять разрешенные для данной роли запросы. Например:
...
Создает [фильтр для строк](../../operations/access-rights.md#row-policy-management), к которым пользователь имеет доступ на чтение.
...
С помощью данной секции вы можете создать политику разрешения или ограничения.
...
Ограничительные политики применяются к строкам, прошедшим разрешительный фильтр. Если не зададите разрешительные политики, пользователь не сможет обращаться ни к каким строкам из таблицы.
...
Ключевым словом `ALL` обозначаются все пользователи, включая текущего. Ключевые слова `ALL EXCEPT` позволяют исключить пользователей из списка пользователей. Например, `CREATE ROW POLICY ... TO ALL EXCEPT accountant, john@localhost`
