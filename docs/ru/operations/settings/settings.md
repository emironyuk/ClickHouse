...
Этот параметр полезен при использовании форматов, требующих определения схемы, например [Капитан Прото](https://capnproto.org/), [Protobuf](https://developers.google.com/protocol-buffers/) или [Шаблон](https://clickhouse.Яндекс / docs / en / интерфейсы / форматы / # шаблон). Значение параметра зависит от формата.
...
При выполнении `ВСТАВЛЯТЬ` запросы, замените пропущенные значения входных столбцов значениями по умолчанию соответствующих столбцов. Этот параметр применяется только к [JSONEachRow](../.. / интерфейсы / форматы. md#jsoneachrow) и [CSV](..|||UNTRANSLATED_CONTENT_START|||/../interfaces/formats.md#csv) formats.|||UNTRANSLATED_CONTENT_END|||

!!! Примечание "Примечание"
Если этот параметр включен, метаданные расширенной таблицы отправляются с сервера на клиент. Он потребляет дополнительные вычислительные ресурсы на сервере и может снизить производительность.
...
Включает или отключает использование значений по умолчанию, если входные данные содержат `НОЛЬ`, но тип данных соответствующего столбца в не `Значение Null(Т)` (для формата CSV).
...
Настройка не применяется к [функция для работы с датой и временем](../../query_language/functions/date_time_functions.md).
...
- `'basic'` — используется базовый парсер.
    ...
    ClickHouse может разобрать только основные `ГГГГ-ММ-ДД ЧЧ: ММ: СС` формат. Например, `'2019-08-20 10: 18: 56'`.
...
- `ALL` — если правая таблица содержит несколько подходящих строк, то ClickHouse выполняет их [декартово произведение](https://ru.wikipedia.org/wiki/Прямое_произведение). Это нормальное поведение `JOIN` как в стандартном SQL.
- `ANY` — если в правой таблице несколько соответствующих строк, то присоединяется только первая найденная. Если в "правой" таблице есть не более одной подходящей строки, то результаты `ANY` и `ALL` совпадают.
- `От` - Для соединения последовательностей с неопределенным совпадением.
...
Этот параметр применяется только для `ПРИСОЕДИНЯТЬСЯ` операции с [Присоединяться](../table_engines/присоединиться.МД) таблицы двигателя.
...
Изменяет поведение `ANY JOIN`. Если выключена, то `ANY JOIN` берёт первую строку найденную для ключа. Если включена, то  `ANY JOIN` берёт последнюю совпавшую строку если одному ключу соответствует множество строк. Настройка работает только для [движка таблиц Join](../table_engines/join.md).
...
Если требуется прочитать более, чем `merge_tree_max_rows_to_use_cache` строк в одном запросе, ClickHouse не используют кэш несжатых блоков. |||UNTRANSLATED_CONTENT_START|||The [uncompressed_cache_size](../server_settings/settings.md#server-settings-uncompressed_cache_size) server setting defines the size of the cache of uncompressed blocks.|||UNTRANSLATED_CONTENT_END|||
...
|||UNTRANSLATED_CONTENT_START|||Queries sent to ClickHouse with this setup are logged according to the rules in the [query_log](../server_settings/settings.md#server_settings-query-log) server configuration parameter.|||UNTRANSLATED_CONTENT_END|||
...
Использование кэша несжатых блоков (только для таблиц семейства MergeTree) может существенно сократить задержку и увеличить пропускную способность при работе с большим количеством коротких запросов. Включите эту настройку для пользователей, от которых идут частые короткие запросы. |||UNTRANSLATED_CONTENT_START|||Also pay attention to the [uncompressed_cache_size](../server_settings/settings.md#server-settings-uncompressed_cache_size) configuration parameter (only set in the config file) – the size of uncompressed cache blocks.|||UNTRANSLATED_CONTENT_END||| По умолчанию - 8 GiB. Кэш разжатых блоков заполняется по мере надобности, а наиболее невостребованные данные автоматически удаляются.
...
|||UNTRANSLATED_CONTENT_START|||Works for tables with streaming in the case of a timeout, or when a thread generates [max_insert_block_size](#settings-max_insert_block_size) rows.|||UNTRANSLATED_CONTENT_END|||
...
- [|||UNTRANSLATED_CONTENT_START|||Random](#load_balancing-random) (by default)|||UNTRANSLATED_CONTENT_END|||
- [|||UNTRANSLATED_CONTENT_START|||Nearest hostname](#load_balancing-nearest_hostname|||UNTRANSLATED_CONTENT_END|||)
- [|||UNTRANSLATED_CONTENT_START|||In order](#load_balancing-in_order|||UNTRANSLATED_CONTENT_END|||)
- [|||UNTRANSLATED_CONTENT_START|||First or random](#load_balancing-first_or_random|||UNTRANSLATED_CONTENT_END|||)
...
### in_order {#load_balancing-in_order}
...
- |||UNTRANSLATED_CONTENT_START|||0 — ClickHouse uses the balancing strategy specified by the [load_balancing](#settings-load_balancing) setting.|||UNTRANSLATED_CONTENT_END|||

Значение по умолчанию — 1.

!!! предупреждение " предупреждение"
Отключите этот параметр, если вы используете [max_parallel_replicas](настройки-max_parallel_replicas #).
...
Для формата ввода CSV включает или отключает синтаксический анализ без кавычек `НОЛЬ` как буквальный (синоним для `\Северный`).

## insert_quorum {#settings-insert_quorum} {#settings-insert_quorum}
...
|||UNTRANSLATED_CONTENT_START|||When reading the data written from the `insert_quorum`, you can use the [select_sequential_consistency](#settings-select_sequential_consistency) option.|||UNTRANSLATED_CONTENT_END|||
...
- [|||UNTRANSLATED_CONTENT_START|||insert_quorum_timeout](#settings-insert_quorum_timeout|||UNTRANSLATED_CONTENT_END|||)
- [|||UNTRANSLATED_CONTENT_START|||select_sequential_consistency](#settings-select_sequential_consistency|||UNTRANSLATED_CONTENT_END|||)
...
- [|||UNTRANSLATED_CONTENT_START|||insert_quorum](#settings-insert_quorum|||UNTRANSLATED_CONTENT_END|||)
- [|||UNTRANSLATED_CONTENT_START|||select_sequential_consistency](#settings-select_sequential_consistency|||UNTRANSLATED_CONTENT_END|||)
...
1. Переписывание запросов на соединение из синтаксиса с запятыми в синтаксис `РЕГИСТРАЦИЯ ВКЛ / С ПОМОЩЬЮ` синтаксис. Если значение параметра равно 0, ClickHouse не обрабатывает запросы с синтаксисом, использующим запятые, и генерирует исключение.
2. |||UNTRANSLATED_CONTENT_START|||Converting `CROSS JOIN` to `INNER JOIN` if `WHERE` conditions allow it.|||UNTRANSLATED_CONTENT_END|||
...
- [|||UNTRANSLATED_CONTENT_START|||Multiple JOIN](..|||UNTRANSLATED_CONTENT_END|||/../query_language/select.md#select-join)
...
Включает или отключает бесшумный пропуск:

- Узел, если его имя не может быть разрешено через DNS.

    Если пропуск отключен, ClickHouse требует, чтобы все узлы [конфигурация кластера](../server_settings/настройки.МД#server_settings_remote_servers) можно разрешить через DNS. В противном случае ClickHouse создает исключение при попытке выполнить запрос в кластере.

    Если пропуск включен, ClickHouse считает неразрешенные узлы недоступными и пытается разрешить их при каждой попытке подключения. Такое поведение создает риск неправильной конфигурации кластера, поскольку пользователь может указать неправильное имя узла, и ClickHouse не сообщает об этом. Однако это может быть полезно в системах с динамическим DNS, например, [Kubernetes](https://kubernetes.io), где узлы могут быть неразрешимы во время простоя, и это не ошибка.

- Осколок, если нет доступных реплик осколка.

    Если пропуск отключен, ClickHouse создает исключение.

    Когда пропуск включен, ClickHouse возвращает частичный ответ и не сообщает о проблемах с доступностью узлов.
