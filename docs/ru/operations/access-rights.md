# Управление доступом пользователей {#access-control}
...
- [Доступ к строкам](#row-policy-management)
...
По умолчанию сервер ClickHouse предоставляет аккаунт пользователя `default`, который не может использовать SQL-ориентированное управление доступом, но у него есть все права и разрешения. Аккаунт `default` используется во всех случаях, когда имя пользователя не определено. Например, при входе с клиента или в распределенных запросах. При распределенной обработке запроса `default` используется, если в конфигурации сервера или кластера не указаны свойства [user и password](../engines/table-engines/special/distributed.md).
...
2. Настройки стандартных ролей аккаунта. Если настройка задана для нескольких ролей, порядок применения не определен.
3. Настройки из профилей настроек, присвоенных пользователю или его стандартным ролям. Если настройка задана в нескольких профилях, порядок применения не определен.
...
- Настройте каталог для хранения конфигураций.
    ...
    ClickHouse хранит конфигурации объектов доступа в папке, установленной в конфигурационном параметре сервера [access_control_path](server-configuration-parameters/settings.md#access_control_path).

- Включите управление доступом на основе SQL как минимум для одного аккаунта.
